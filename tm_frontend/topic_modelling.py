...
if selected == "Home":
    ...
    st.image("images/intro.jpeg", width=820)
...
if selected == "GSDMM":
    ...
    movie_group_approach = """
        ...
        <li> <span class='highlight'>Rinse and repeat until you reach a plateau where the number of clusters does not change. </span> </li>
        </ul>
        """
...
if selected == "References":
    ...
    # We only need to cite the papers and not display them as there maybe
    # legal issues if we render them directly

    # st.header("Read the papers here")

    # def show_pdf(file_path):
    #     with open(file_path, "rb") as f:
    #         base64_pdf = base64.b64encode(f.read()).decode('utf-8')
    #     pdf_display = f"""
    #         <iframe src = "data:application/pdf;base64,{base64_pdf}" width = "800"
    #         height = "800" type = "application/pdf" > </iframe >
    #         """
    # st.markdown(pdf_display, unsafe_allow_html=True)

    # with st.expander("Topic Modelling Survey", expanded=False):
    #     show_pdf('papers/topic_modelling_paper.pdf')

    # with st.expander("GSDMM", expanded=False):
    #     show_pdf('papers/gsdmm_paper.pdf')

    # with st.expander("BERTopic", expanded=False):
